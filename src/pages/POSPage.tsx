...
import { Plus, Minus, Trash2, Check, Printer, Settings } from 'lucide-react'
...
export function POSPage() {
  ...
  const { barbers, services, products } = useConfigStore()
  ...
  const commission = barber ? subtotal * barber.commissionRate : 0

  const handleCheckout = () => {
    if (cart.length === 0 || !barber) return
    const sale = addSale(cart, method, barber.id, barber.name, barber.commissionRate)
    setReceipt(sale)
    setCart([])
  }

  // Empty state
  if (barbers.length === 0 || (services.length === 0 && products.length === 0)) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-8">
        ...
        <p className="text-white/30 text-sm mb-6 max-w-xs">
          Agregá barberos y al menos un servicio o producto para usar el POS.
        </p>
        <Link to="/app/settings" className="px-5 py-2.5 bg-white text-black text-sm font-semibold rounded hover:bg-white/90 transition-colors">
          Ir a Configuración
        </Link>
      </div>
    )
    ...
    return (
      <div className="h-full flex items-center justify-center p-6">
        <div className="bg-[#0a0a0a] border border-white/15 rounded-lg w-full max-w-sm p-8 text-center">
          ...
          <h2 className="text-white font-black text-xl uppercase tracking-tight mb-1">Venta registrada</h2>
          <p className="text-white/30 text-sm mb-6">
            {format(new Date(receipt.date), "d MMM yyyy · HH:mm", { locale: es })}
          </p>
          ...
          <div className="text-left space-y-2 mb-6 border border-white/10 rounded p-4">
            ...
            <div className="border-t border-white/10 pt-2 mt-2 space-y-1">
              <div className="flex justify-between text-sm">
                <span className="text-white/30">Comisión ({barber ? Math.round(barber.commissionRate * 100) : 0}%)</span>
                <span className="text-white/50">-{fmt(receipt.commission)}</span>
              </div>
              <div className="flex justify-between font-bold">
                <span className="text-white">Neto barbería</span>
                <span className="text-white text-lg">{fmt(receipt.net)}</span>
              </div>
            </div>
          </div>

          <div className="flex items-center justify-between text-sm text-white/30 mb-6 px-1">
            <span>{receipt.barberName}</span>
            <span>{receipt.method === 'CASH' ? 'Efectivo' : receipt.method === 'TRANSFER' ? 'Transferencia' : 'Tarjeta'}</span>
          </div>

          <div className="flex gap-3">
            <button className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-white/15 text-white/40 text-sm rounded hover:border-white/30 transition-colors">
              <Printer size={14} /> Imprimir
            </button>
            <button onClick={() => setReceipt(null)} className="flex-1 py-2.5 bg-white text-black text-sm font-semibold rounded hover:bg-white/90 transition-colors">
              Nueva venta
            </button>
          </div>
        </div>
    ...
    <div className="h-full flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden border-r border-white/10">
        <div className="h-14 border-b border-white/10 flex items-center px-6 gap-6 flex-shrink-0">
          <h1 className="text-sm font-black uppercase tracking-tight">Punto de Venta</h1>
          <div className="flex gap-1 bg-white/5 border border-white/10 rounded p-1">
            {(['services', 'products'] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={cn('px-3 py-1 text-xs rounded transition-colors', tab === t ? 'bg-white text-black font-semibold' : 'text-white/40 hover:text-white')}
              >
        ...
        <div className="flex-1 overflow-auto p-4">
          {tab === 'services' && services.length === 0 && (
            <p className="text-white/20 text-sm p-4">Sin servicios configurados.</p>
          )}
          {tab === 'products' && products.length === 0 && (
            <p className="text-white/20 text-sm p-4">Sin productos configurados.</p>
          )}
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {(tab === 'services' ? services : products).map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => addItem(item.id, item.name, item.price, item.isService ? 'service' : 'product')}
                ...
                >
                  ...
                  {'stock' in item && (
                    <p className={cn('text-xs mt-2', item.stock <= item.minStock ? 'text-red-400' : 'text-white/20')}>
                      Stock: {item.stock}
                    </p>
                  )}
      ...
      <div className="w-80 flex-shrink-0 flex flex-col">
        ...
        <div className="flex-1 overflow-auto p-4">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-white/20">
              ...
              <p className="text-xs mt-1">Seleccioná servicios o productos</p>
            </div>
          ) : (
              ...
              {cart.map((item) => (
                <div key={item.id} className="flex items-center gap-3 py-2 border-b border-white/5">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/30 text-xs">{fmt(item.price)} × {item.quantity}</p>
                  </div>
        ...
        <div className="p-4 border-t border-white/10 flex-shrink-0 space-y-4">
          <div className="space-y-2 text-sm">
            ...
            {barber && <div className="flex justify-between text-white/30"><span>Comisión ({Math.round(barber.commissionRate * 100)}%)</span><span>-{fmt(commission)}</span></div>}
            <div className="flex justify-between text-white font-bold text-base"><span>Neto Barberia</span><span>{fmt(subtotal - commission)}</span></div>
          </div>

          <div className="grid grid-cols-3 gap-1">
            {([['CASH', 'Efectivo'], ['TRANSFER', 'Transfer'], ['CARD', 'Tarjeta']] as [PaymentMethod, string][]).map(([m, label]) => (
              <button key={m} onClick={() => setMethod(m)} className={cn('py-2 text-xs rounded border transition-colors', method === m ? 'bg-white text-black border-white font-semibold' : 'border-white/15 text-white/40 hover:border-white/30 hover:text-white')}>
                {label}
              </button>
            ))}
          </div>

          <button onClick={handleCheckout} disabled={cart.length === 0} className="w-full py-3 bg-white text-black font-bold text-sm rounded hover:bg-white/90 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
